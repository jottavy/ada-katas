...
const choisirEmoji = (conditions, temperature) => {
  if (temperature > 25 && conditions === "ensoleille" ) {
  return "🌞"   
  }
...
const decrireMeteo = (conditions, temperature) => {
  let resultat = ""
  if (temperature < 0) {
  resultat = "Temps glacial"
  }
  else if (temperature < 10) {
  resultat = "Temps froid"
  }
  else if (temperature < 20) {
  resultat = "Temps frais"
  }
  else if (temperature < 30) {
  resultat = "Temps agréable"
  }
  else {
  resultat = "Temps chaud"
  }
  if (conditions === "pluvieux" || conditions === "orageux") {
  resultat += " et pluvieux"
  }
  if (conditions === "neige") {
  resultat += " et enneigé"
  }
